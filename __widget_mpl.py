...
class ProTracerDialog(QtWidgets.QDialog):
    def __init__(self, is2d=True, parent=None):
        ...
        if is2d:
            ...
            self.canvas = PlotCanvas3D(self, width=5, height=10)
    ...
    def on_draw_2d(self):
        ...
        for i in range(len(self.shots)):
            self.pt.add_plot_data(self.shots[i][0], self.shots[i][1])
    ...
    def plot_3d(self, azim=45, axis_on=False, title="ProTracer 3D"):
        ...
        self.canvas.ax.view_init(elev=0, azim=azim)
        self.canvas.ax._axis3don = axis_on
        self.canvas.ax.set_axis_off()
